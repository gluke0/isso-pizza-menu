<script>

export default {
  name: "pizza",
  data() {
    return {
    }
  },
  components: {
    
  },
}
</script>

<template>
    
    <div>
      <h3 class="text-uppercase mt-3">pizze <span class="detmain">(prezzo a porzione)</span></h3>
      <div class="">
        <div>
            <div class="d-flex justify-content-between">
              <li class="oddy">Pizza Margherita</li> <span class="oddy">€ 5,00</span>
            </div>
            <span>(mozzarella, pomodoro)</span>

            <div class="d-flex justify-content-between">
              <li>Pizza Rucola</li> <span class="even">€ 5,50</span>
            </div>
            <span>(mozzarella, pomodoro, rucola)</span>

            <div class="d-flex justify-content-between">
              <li class="oddy">Pizza Cipolla</li> <span class="oddy">€ 5,50</span>
            </div>
            <span>(mozzarella, pomodoro, cipolla)</span>

            <div class="d-flex justify-content-between">
              <li>Pizza Capperi</li> <span class="even">€ 5,50</span>
            </div>
            <span>(mozzarella, pomodoro, capperi)</span>

            <div class="d-flex justify-content-between">
              <li class="oddy">Pizza Olive</li> <span class="oddy">€ 5,50</span>
            </div>
            <span>(mozzarella, pomodoro, olive)</span>

            <div class="d-flex justify-content-between">
              <li>Pizza Acciughe</li> <span class="even">€ 5,50</span>
            </div>
            <span>(mozzarella, pomodoro, acciughe)</span>

            <div class="d-flex justify-content-between">
              <li class="oddy">Pizza Carciofi</li> <span class="oddy">€ 6,00</span>
            </div>
            <span>(mozzarella, pomodoro, carciofi)</span>

            <div class="d-flex justify-content-between">
              <li>Pizza Wurstel</li> <span class="even">€ 6,00</span>
            </div>
            <span>(mozzarella, pomodoro, wurstel)</span>

            <div class="d-flex justify-content-between">
              <li class="oddy">Pizza al Prosciutto Cotto</li> <span class="oddy">€ 6,00</span>
            </div>
            <span>(mozzarella, pomodoro, prosciutto cotto)</span>

            <div class="d-flex justify-content-between">
              <li>Pizza Patate Fritte</li> <span class="even">€ 6,00</span>
            </div>
            <span>(mozzarella, pomodoro, patate fritte)</span>

            <div class="d-flex justify-content-between">
              <li class="oddy">Pizza Capricciosa</li> <span class="oddy">€ 6,50</span>
            </div>
            <span>(mozzarella, pomodoro, olive, capperi, peperoni, funghi, carciofi)</span>

            <div class="d-flex justify-content-between">
              <li>Pizza Prosciutto e Funghi</li> <span class="even">€ 7,00</span>
            </div>
            <span>(mozzarella, pomodoro, prosciutto cotto, funghi champignon)</span>

            <div class="d-flex justify-content-between">
              <li class="oddy">Pizza Wurstel e Patatine</li> <span class="oddy">€ 7,00</span>
            </div>
            <span>(mozzarella, pomodoro, wurstel, patatine fritte)</span>

            <div class="d-flex justify-content-between">
              <li>Pizza 4 Stagioni</li> <span class="even">€ 7,50</span>
            </div>
            <span>(mozzarella, pomodoro, olive, capperi, funghi, peperoni, carciofi, prosciutto)</span>

            <div class="d-flex justify-content-between">
              <li class="oddy">Pizza Pancetta e Tonno</li> <span class="oddy">€ 7,50</span>
            </div>
            <span>(mozzarella, pomodoro, pancetta, tonno)</span>

            <div class="d-flex justify-content-between">
              <li>Pizza Mapy</li> <span class="even">€ 7,50</span>
            </div>
            <span>(mozzarella, pomodoro, rucola, scaglie di grana, prosciutto crudo)</span>

            <div class="d-flex justify-content-between">
              <li class="oddy">Pizza Tonno</li> <span class="oddy">€ 6,00</span>
            </div>
            <span>(mozzarella, pomodoro, tonno)</span>

            <div class="d-flex justify-content-between">
              <li>Pizza Funghi Champignon</li> <span class="even">€ 6,00</span>
            </div>
            <span>(mozzarella, pomodoro, funghi champignon)</span>

            <div class="d-flex justify-content-between">
              <li class="oddy">Pizza Prosciutto Crudo</li> <span class="oddy">€ 6,50</span>
            </div>
            <span>(mozzarella, pomodoro, prosciutto crudo)</span>

            <div class="d-flex justify-content-between">
              <li>Pizza Salame Piccante</li> <span class="even">€ 6,50</span>
            </div>
            <span>(mozzarella, pomodoro, salame piccante)</span>

            <div class="d-flex justify-content-between">
              <li class="oddy">Pizza Speck</li> <span class="oddy">€ 6,50</span>
            </div>
            <span>(mozzarella, pomodoro, speck)</span>

            <div class="d-flex justify-content-between">
              <li>Pizza Salame</li> <span class="even">€ 6,50</span>
            </div>
            <span>(mozzarella, pomodoro, salame)</span>

            <div class="d-flex justify-content-between">
              <li class="oddy">Pizza Pancetta</li> <span class="oddy">€ 6,50</span>
            </div>
            <span>(mozzarella, pomodoro, pancetta)</span>

            <div class="d-flex justify-content-between">
              <li>Pizza Verdure Grigliate</li> <span class="even">€ 6,50</span>
            </div>
            <span>(mozzarella, pomodoro, verdure grigliate miste)</span>

            <div class="d-flex justify-content-between">
              <li class="oddy">Pizza Funghi Misti</li> <span class="oddy">€ 6,50</span>
            </div>
            <span>(mozzarella, pomodoro, funghi misti)</span>

            <div class="d-flex justify-content-between">
              <li>Pizza Tonno e Cipolla</li> <span class="even">€ 6,00</span>
            </div>
            <span>(mozzarella, pomodoro, tonno, cipolla)</span>  
        </div>
      </div>
    </div>

</template>

<style lang="scss">
div span{
  color: rgb(237, 232, 232);
  font-size: smaller;
  font-style: italic;
}

.oddy{
  color: rgb(50, 84, 87);
}

.even{
  color: black;
  font-size: larger;
  font-weight: bold;
}

.oddy, li{
  font-weight: bold;
  font-size: larger;
}

</style>